...
export class CommentSectionComponent {
  ...
  insertMention(user: User) {
    ...
    const newContent =
      textBeforeCursor.slice(0, atIndex) + `@${user.firstName} ${user.lastName} ` + textAfterCursor;
    ...
    if (this.textareaRef?.nativeElement) {
      ...
      const newPosition =
        textBeforeCursor.slice(0, atIndex).length + `@${user.firstName} ${user.lastName} `.length;
