...
export class SearchComponent implements OnInit, OnDestroy {
  ...
  private loadAllUserContent(): void {
    ...
    this.projectsService.loadProjects()
        ...
        mergeMap((projects: Project[]) => {
          ...
          const taskObservables = projects.map(project =>
            this.tasksService.getTasksByProject(project.id).pipe(
              catchError((error) => {
                this.logger.error('Failed to load tasks for project ' + project.name + ': ' + error.message);
                return of({ data: [] });
              })
            )
          );
          ...
          return forkJoin(taskObservables).pipe(
            mergeMap((tasksResults) => {
              // Flatten all tasks
              const allTasks: Task[] = tasksResults
              ...
              const commentObservables = allTasks.map(task =>
                this.commentsService.getCommentsByTask(task.id).pipe(
                  ...
                  catchError((error) => {
                    this.logger.error('Failed to load comments for task ' + task.title + ': ' + error.message);
                    return of([]);
                  })
                )
              );
      ...
      .subscribe((data: any) => {
        ...
        this.logger.info(
          'Loaded content - Projects: ' + projects.length +
  ...
  private filterContent(query: string): void {
    ...
    const filtered_tasks = this.allTasks().filter((task) =>
    ...
    const filtered_projects = this.allProjects().filter((project) =>
    ...
    this.logger.info(
      'Filter results - Tasks: ' + filtered_tasks.length + 
      ', Projects: ' + filtered_projects.length + 
      ', Comments: ' + filtered_comments.length
    );
  }
  ...
  get showNoResults(): boolean {
    return this.isSearchActive() && 
           !this.isLoading() && 
           !this.hasFilteredResults();
  }

  get totalResults(): number {
    return this.filteredTasks().length + this.filteredProjects().length + this.filteredComments().length;
  }
